...
def _load_from(base: Path):
    ocr_cand = json.loads((base / "ocr_candidate_result.json").read_text(encoding="utf-8"))
    ref_cand = json.loads((base / "ref_candidate_result.json").read_text(encoding="utf-8"))
    ...
    cand_map = json.loads((base / "maps" / "candidate_map.json").read_text(encoding="utf-8"))
...
def build_candidate_df(ocr_cand, ref_cand, cand_map):
    ...
    for cid in all_ids:
        ...
        rows.append({
            "#": cid, "label": f"#{cid} {name}", "Candidate": name, "Party": party,
            "OCR": ocr, "Ref": ref, "Diff": ocr - ref, "Error %": round(err, 2), "phantom": False,
        })
...
def build_party_df(ocr_party, ref_party, party_map):
    ...
    for pid in all_ids:
        ...
        err = pct_error(ocr, ref)
        rows.append({
            "#": pid, "label": f"#{pid} {name}", "Party": name,
            "OCR": ocr, "Ref": ref, "Diff": ocr - ref,
            "Error %": round(err, 2) if not phantom else None, "phantom": phantom,
        })
    return pd.DataFrame(rows)
...
def _grouped_bar_fig(normal, phantoms, title):
    ...
    fig.add_trace(go.Bar(
        name="OCR", x=normal["label"], y=normal["OCR"], marker_color="#4C78A8",
    ...
    fig.add_trace(go.Bar(
        name="Reference", x=normal["label"], y=normal["Ref"], marker_color="#F58518",
    ...
    if not phantoms.empty:
        fig.add_trace(go.Bar(
            name="OCR (phantom — no ref)", x=phantoms["label"], y=phantoms["OCR"],
            marker_color="#E45756",
            hovertemplate="<b>%{x}</b><br>OCR: %{y:,}<br>⚠️ No reference entry<extra></extra>",
        ))
    fig.update_layout(
        title=title, barmode="group", xaxis_tickangle=-40, height=350,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        hovermode="closest", margin=dict(b=100),
    )
    return fig
...
def error_bar_chart(df, title):
    ...
    fig = go.Figure(go.Bar(
        x=data["Error %"], y=data["label"], orientation="h", marker_color=colors,
    ...
    fig.update_layout(
        title=title, xaxis_title="% Error  |OCR − Ref| / Ref × 100",
        height=max(300, len(data) * 20 + 80),
        shapes=[
            dict(type="line", x0=1, x1=1, y0=0, y1=1, yref="paper",
                 line=dict(color="#ff7f0e", dash="dash", width=1.5)),
            dict(type="line", x0=5, x1=5, y0=0, y1=1, yref="paper",
                 line=dict(color="#d62728", dash="dash", width=1.5)),
        ],
        annotations=[
            dict(x=1, y=1, yref="paper", text="1%", showarrow=False,
                 font=dict(color="#ff7f0e", size=11), xanchor="left", yanchor="bottom"),
            dict(x=5, y=1, yref="paper", text="5%", showarrow=False,
                 font=dict(color="#d62728", size=11), xanchor="left", yanchor="bottom"),
        ],
        margin=dict(l=20),
    )
    return fig
...
def metrics_row(df, label):
    ...
    c2.metric("Total Ref Votes", f"{total_ref:,}", delta=f"{total_diff:+,}", delta_color="off")
...
def render_dataset_tab(ocr_cand, ref_cand, ocr_party, ref_party, cand_map, party_map):
    ...
    with sub_cand:
        ...
        with col1:
            st.plotly_chart(grouped_bar_chart(cand_df, "Candidate Votes: by ID"), use_container_width=True)
        with col2:
            st.plotly_chart(votes_bar_chart(cand_df, "Candidate Votes: by Vote Count"), use_container_width=True)
        st.plotly_chart(error_bar_chart(cand_df, "Candidate % Error"), use_container_width=True)
    ...
    with sub_party:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(grouped_bar_chart(party_df, "Party Votes: by ID"), use_container_width=True)
        with col2:
            st.plotly_chart(votes_bar_chart(party_df, "Party Votes: by Vote Count"), use_container_width=True)
        st.plotly_chart(error_bar_chart(party_df, "Party % Error"), use_container_width=True)
        with st.expander("Data Table"):
            display = party_df[["#", "Party", "OCR", "Ref", "Diff", "Error %", "phantom"]].copy()
            display.columns = ["#", "Party", "OCR", "Ref", "Diff", "Error %", "Phantom ⚠️"]

            def _highlight_phantom(row):
                return ["background-color: #ffe0e0"] * len(row) if row["Phantom ⚠️"] else [""] * len(row)
...
st.caption("Comparing aggregated OCR results against ground-truth reference data. Error = |OCR − Ref| / Ref × 100%.")
...
with tab_compare:
    ...
    def _summary(cand_df, party_df):
        ...
        return {
            "cand_total_err": pct_error(int(cand_df["OCR"].sum()), int(cand_df["Ref"].sum())),
            "cand_mean_err": cand_valid["Diff"].abs().sum() / cand_valid["Ref"].sum() * 100,
            "party_total_err": pct_error(
                int(party_df[~party_df["phantom"]]["OCR"].sum()),
                int(party_df[~party_df["phantom"]]["Ref"].sum()),
            ),
            "party_mean_err": party_valid["Diff"].abs().sum() / party_valid["Ref"].sum() * 100,
        }
    ...
    metrics_labels = ["Candidate Weighted Mean Error %", "Candidate Total Error %", "Party Weighted Mean Error %", "Party Total Error %"]
    kan_vals = [kan_s["cand_mean_err"], kan_s["cand_total_err"], kan_s["party_mean_err"], kan_s["party_total_err"]]
    aom_vals = [aom_s["cand_mean_err"], aom_s["cand_total_err"], aom_s["party_mean_err"], aom_s["party_total_err"]]

    fig = go.Figure()
    fig.add_trace(go.Bar(
        name="Kan", x=metrics_labels, y=kan_vals, marker_color="#4C78A8",
        text=[f"{v:.2f}%" for v in kan_vals], textposition="outside",
    ))
    fig.add_trace(go.Bar(
        name="Aomsin", x=metrics_labels, y=aom_vals, marker_color="#F58518",
        text=[f"{v:.2f}%" for v in aom_vals], textposition="outside",
    ))
    fig.update_layout(
        title="Error Metrics: Kan vs Aomsin", barmode="group", yaxis_title="% Error",
