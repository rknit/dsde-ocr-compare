...
PARTY_NUM_KEYS = {
    "หมายเลขบัญชีรายชื่อของพรรคการเมือง",
    "หมายเลขบัญศยรายชื่อของพรรคการเมือง",
    "หมายเลขบัญษีรายชื่อของพรรคการเมือง",
    "หมายเลขบัญบัญชีรายชื่อของพรรคการเมือง",
}


def get_vote_value(item, filepath):
    if "คะแนน" in item:
        return item["คะแนน"]
    if "คะคะแนน" in item:
        return item["คะคะแนน"]
    raise ValueError(f"[ERROR] No vote value key in {filepath}: {list(item.keys())}")


def aggregate_candidates():
    totals = defaultdict(int)
    files = sorted(glob.glob(str(BASE / "normal" / "**" / "*.json"), recursive=True))
    for filepath in files:
        data = json.loads(Path(filepath).read_text(encoding="utf-8"))
        for item in data.get("ผลคะแนน", []):
            candidate_num = item["หมายเลขประจำตัวผู้สมัคร"]
            votes = get_vote_value(item, filepath)
            totals[candidate_num] += votes
    return totals, len(files)


def aggregate_parties():
    totals = defaultdict(int)
    files = sorted(glob.glob(str(BASE / "bch" / "**" / "*.json"), recursive=True))
    for filepath in files:
        data = json.loads(Path(filepath).read_text(encoding="utf-8"))
        for item in data.get("ผลคะแนนพรรค", []):
            matched_key = next((k for k in item if k in PARTY_NUM_KEYS), None)
            if matched_key is None:
                raise ValueError(f"[ERROR] Unknown party number key in {filepath}: {list(item.keys())}")
            party_num = item[matched_key]
            votes = get_vote_value(item, filepath)
            totals[party_num] += votes
    return totals, len(files)
...
def main():
    ...
    print("Aggregating candidate votes from normal/...")
    ocr_candidates, n_normal = aggregate_candidates()
    print(f"  Processed {n_normal} files.")

    print("Aggregating party votes from bch/...")
    ocr_parties, n_bch = aggregate_parties()
    print(f"  Processed {n_bch} files.")
